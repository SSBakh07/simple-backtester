...
class SBDataSrc:
    ...
    def __init__(
        ...
        if data.index.inferred_type != "datetime64":
            try:
                data[self.date_col] = pd.to_datetime(data[self.date_col], format=date_fmt)
            except ValueError:
                raise ValueError("Invalid data format for data source. Specify date format using date_fmt parameter.")
